...
export function isSus(
	...
	if (typeof lineData !== "undefined") {
		if (
			lineData.code !== trip.routeId ||
			vehicle.Direction - 1 !== trip.directionId
		) {
			console.warn(
		...
		if (
			...
			console.warn(
				"\t\t  Missing from old GTFS-RT, and destination is unknown: probably sus.",
			);
			return true;
		}
	}
	...
	if (typeof oldVehiclePosition !== "undefined") {
		...
		if (trip.routeId !== oldTrip.routeId) {
			console.warn(
				"\t\t  Mismatch between matched trip and old GTFS-RT routes: is sus.",
			);
			console.warn(
				`\t\t  Matched trip: ${trip.routeId} | Old GTFS-RT: ${oldTrip.routeId}`,
			);
			return true;
		}

		if (
			typeof lineData !== "undefined" &&
			!lineData.destinations.includes(vehicle.Destination)
		) {
			console.warn(
	...
	if (
		...
		if (
			!lineData.destinations.includes(vehicle.Destination) &&
			(trip.routeId !== oldTrip.routeId ||
				trip.directionId !== (oldTrip.directionId ?? 0))
		) {
			console.warn(
