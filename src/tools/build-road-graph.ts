...
/** Les valeurs de `highway` qu'un autobus peut emprunter. */
...
/**
 * Les sens praticables d'un chemin, ou `undefined` s'il n'est pas une route.
 *
 * Le parti pris est celui d'un autobus, et non d'une voiture : un couloir à contresens, une voie
 * réservée, une rue interdite « sauf bus » font partie du réseau. C'est précisément là que passent
 * les déviations, et les écarter obligerait à les redessiner à la main à chaque fois.
 */
function roadDirections(tags: Map<string, string>): number | undefined {
	const highway = tags.get("highway");
	if (highway === undefined || !DRIVABLE.has(highway)) return undefined;

	// Une aire — parking, place — est une surface, pas un axe : sa géométrie ferme un contour et
	// router dessus donnerait des tours de pâté de maisons.
	if (tags.get("area") === "yes") return undefined;

	const access = tags.get("access");
	if (access === "no" || access === "private") {
		const transit = tags.get("bus") ?? tags.get("psv");
		if (transit !== "yes" && transit !== "designated") return undefined;
	}
	...
	if (oneway === "yes" || oneway === "1" || oneway === "true") backward = false;
	else if (oneway === "-1" || oneway === "reverse") forward = false;
	else if (oneway === undefined) {
		// Deux cas où le sens unique va sans dire et n'est donc presque jamais écrit.
		const junction = tags.get("junction");
		if (junction === "roundabout" || junction === "circular" || highway === "motorway") backward = false;
	}

	// … sauf pour les bus, justement : « oneway:bus=no » rouvre la rue dans les deux sens.
	if (!forward || !backward) {
		const transit = tags.get("oneway:bus") ?? tags.get("oneway:psv");
		if (transit === "no") {
			forward = true;
			backward = true;
		}
	...
	return (forward ? EDGE_FORWARD : 0) | (backward ? EDGE_BACKWARD : 0);
}
...
function main(): void {
	...
	const wayFlags = new Growable<Uint8Array>((size) => new Uint8Array(size));
	...
	readOsmPbf(input, {
		...
		onWay(tags, readReferences) {
			const directions = roadDirections(tags);
	...
	const graph = buildEdges({ nodeLatitudes, nodeLongitudes, wayNodes, wayStart, wayFlags, usage });
...
function buildEdges(source: {
	...
	const edgeFlags = new Growable<Uint8Array>((size) => new Uint8Array(size));
	...
	for (let way = 0; way < wayCount; way += 1) {
		...
		const flags = source.wayFlags.at(way);
...
/**
 * La liste d'adjacence : pour chaque carrefour, les arêtes qu'on peut en prendre.
 *
 * Une arête y figure une fois par sens praticable, et son rang porte le sens dans son bit de poids
 * faible : l'explorateur sait ainsi, sans rien relire, s'il la parcourt à l'endroit ou à l'envers.
 */
function buildArcs(graph: BuiltGraph): BuiltArcs {
	...
	for (let edge = 0; edge < edgeCount; edge += 1) {
		const flags = graph.edgeFlags.at(edge);
		if ((flags & EDGE_FORWARD) !== 0) counts[graph.edgeFrom.at(edge)] = counts[graph.edgeFrom.at(edge)]! + 1;
		if ((flags & EDGE_BACKWARD) !== 0) counts[graph.edgeTo.at(edge)] = counts[graph.edgeTo.at(edge)]! + 1;
	}
	...
	for (let edge = 0; edge < edgeCount; edge += 1) {
		const flags = graph.edgeFlags.at(edge);
		if ((flags & EDGE_FORWARD) !== 0) {
			const node = graph.edgeFrom.at(edge);
			arcEdges[cursor[node]!] = edge * 2;
			cursor[node] = cursor[node]! + 1;
		}
		if ((flags & EDGE_BACKWARD) !== 0) {
			const node = graph.edgeTo.at(edge);
			arcEdges[cursor[node]!] = edge * 2 + 1;
			cursor[node] = cursor[node]! + 1;
		}
	}
...
function write(
	...
	const place = (section: keyof typeof layout.offsets, values: NumberArray): void => {
		...
		view.set(values as unknown as Uint8Array);
	};
	...
	place("edgeFlags", graph.edgeFlags.view());
