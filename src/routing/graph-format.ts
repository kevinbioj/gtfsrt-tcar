...
/** À incrémenter dès que la disposition change : un fichier d'une autre version est REFUSÉ, pas deviné. */
...
export type GraphCounts = {
	...
	/** Arcs sortants, tous nœuds confondus : au plus deux par arête. */
	arcCount: number;
	gridColumns: number;
	gridRows: number;
...
const SECTIONS: readonly SectionShape[] = [
	...
	{ name: "edgeFlags", width: 1, length: (c) => c.edgeCount },
...
/**
 * Les sens praticables d'une arête. Une arête est toujours décrite dans le sens de son chemin
 * d'origine ; ces deux bits disent lesquels un véhicule peut emprunter.
 */
export const EDGE_FORWARD = 1;
export const EDGE_BACKWARD = 2;
