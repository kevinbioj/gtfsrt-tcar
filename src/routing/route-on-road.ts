...
import { EDGE_BACKWARD, EDGE_FORWARD } from "./graph-format.js";
 ...
 * celles qu'on peut atteindre, sens uniques compris — pour le coût du bout d'arête parcouru, et
 * s'achève symétriquement. Le cas où les deux points partagent la même arête se traite à part : il
 * n'y a rien à chercher, il suffit de longer la rue.
 *
 * Le coût est la seule distance. Aucune préférence n'est donnée aux grands axes : un autobus dévié
 * emprunte volontiers une rue secondaire, et c'est au dessinateur de trancher en posant un point de
 * passage — lui sait ce que l'exploitation a décidé, le graphe non.
 */
...
/** Les deux points sur la même arête, dans un sens praticable : la rue elle-même fait l'itinéraire. */
function alongOneEdge(graph: RoadGraph, start: RoadSnap, end: RoadSnap): RoadRoute | undefined {
	...
	const flags = graph.edgeFlags[start.edge]!;
	const forward = end.distance >= start.distance;
	if (forward && (flags & EDGE_FORWARD) === 0) return undefined;
	if (!forward && (flags & EDGE_BACKWARD) === 0) return undefined;
...
function search(graph: RoadGraph, start: RoadSnap, end: RoadSnap, maxExpansions: number): Arc[] | undefined {
	...
	const endFlags = graph.edgeFlags[end.edge]!;
	...
	// bout de l'arête d'arrivée. Une arête peut s'aborder par ses deux extrémités.
	const exitCost = (node: number): { cost: number; arc: Arc } | undefined => {
		if (node === graph.edgeFrom[end.edge]! && (endFlags & EDGE_FORWARD) !== 0) {
			return { cost: end.distance, arc: { edge: end.edge, reversed: false } };
		}
		if (node === graph.edgeTo[end.edge]! && (endFlags & EDGE_BACKWARD) !== 0) {
